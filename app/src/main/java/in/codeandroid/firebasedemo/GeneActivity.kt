...
import kotlinx.android.synthetic.main.activity_gene.*
import java.time.LocalDateTime
import java.time.format.DateTimeFormatter
import java.util.ArrayList
@RequiresApi(Build.VERSION_CODES.O)

class GeneActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter.setOnClickListener(object : CustomAdapter.OnItemClickListener {
            override fun onItemClick(item: View?, position: Int) {
                val str: String = items[position].text1.toString() + " - "
                ...
                val Intent = Intent(this@GeneActivity, ShowplantActivity::class.java);
